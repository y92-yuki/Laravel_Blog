'use strict';
    ...
    window.addEventListener('DOMContentLoaded',() => {
        ...
        const commentLikeToggle = document.querySelectorAll('.commentLike-toggle');
        const commentDelete = document.querySelectorAll('input[name=not_delete]');

        //コメント削除ボタンを非表示
            for (let p of commentDelete) {
                if (p.value) {
                    p.nextElementSibling.querySelector('a.btn-danger').classList.add('d-none');
                }
            }

        if (postExistsLike.dataset.not_logged_in === 'not_logged_in') {
            postExistsLike.textContent = '*ログインするといいねやコメントを残すことができます'
            postExistsLike.classList.add('h5','text-right','text-secondary')

            //コメントのいいね・取消ボタンを非表示
            for (let p of commentLikeToggle) {
